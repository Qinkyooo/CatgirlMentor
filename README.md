...
- **数据更新**：在线物品数据按字段结构校验，接受兼容的 schema 与数据版本更新；离线知识库仍锚定于构建时点。各工具的刷新与恢复范围见 [数据源更新说明](docs/ffxiv-source-updates.md)。
...
- `tools.games.pvpRulesUrl` 可选：配置后会定期拉取上游快照，**结构与自身摘要验证通过后自动采用新规则**；网络失败使用本次运行中上次有效规则并标注过期。未配置时保持离线推算。
