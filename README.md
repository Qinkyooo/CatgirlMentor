...
本仓库 = **上游 nanobot 框架** + **一层 FFXIV（最终幻想14）游戏域定制**。
关于nanobot本体相关资料请看：https://nanobot.wiki/cn/docs/0.3.0/getting-started/start-without-technical-background

- **框架层**（沿用上游能力，不重复造轮子）：WebUI / 终端 TUI、QQ / Telegram 等聊天渠道、Dream 长短期记忆、MCP 工具、定时任务、子代理、OpenAI 兼容 API 等，全部可用。
- **增量层**（本仓库的全部自定义，即“二次开发”）：
  - 默认人设改为「艾欧泽亚猫娘导师」，称呼玩家为「小豆芽」；
  - 新增 4 个只读 FF14 工具：钓鱼、中文知识库、房区、物价；
  - 内置约 8.7MB 的中文攻略知识库（随源码分发，首次启动自动校验绑定）；
  - 一套「Persona 人设工作流」，可基于剧情台词与 Wiki 素材扮演任意角色；
  - 聊天频道默认静默，不在 QQ 群里刷进度噪音。
...
- 🎭 **Persona 人设系统**：证据驱动地生成角色档案（素材收集 → 预览 → 应用），随时切换人设，也可一键退出恢复默认。
- 🤫 **安静的默认值**：`sendProgress` / `sendToolHints` 默认关闭、QQ 频道不再回复 “Processing…” 占位——机器人只在你真正提问时开口。
